...
def _write_project_files(blueprint_text: str, tools: list[dict], output_dir: str) -> Path:
    ...
    if env_match:
        for line in env_match.group(1).strip().split("\n"):
            ...
            if m:
                ...
                # Strip NEXT_PUBLIC_ prefix from credential vars — they must stay server-side
                _private_suffixes = (
                    "DATABASE_URL", "MONGODB_URI", "REDIS_URL", "SECRET_KEY", "SERVICE_ROLE_KEY",
                    "AUTH_TOKEN", "WEBHOOK_SECRET", "PRIVATE_KEY", "API_KEY", "API_SECRET",
                )
                if var_name.startswith("NEXT_PUBLIC_") and any(var_name.endswith(s) or s in var_name for s in _private_suffixes):
                    var_name = var_name[len("NEXT_PUBLIC_"):]
                    comment = (comment + " (server-side only — do not expose to client)").strip()
                if comment:
    ...
    if not env_lines:
        ...
        known: dict[str, str] = {
            ...
            "Supabase":          "NEXT_PUBLIC_SUPABASE_URL=https://<project>.supabase.co\nNEXT_PUBLIC_SUPABASE_ANON_KEY=\nSUPABASE_SERVICE_ROLE_KEY=",
        ...
        # Credentials that must NEVER be NEXT_PUBLIC_ (would leak to client bundle)
        _private_var_patterns = (
            "DATABASE_URL", "MONGODB_URI", "REDIS_URL", "SECRET_KEY", "SERVICE_ROLE_KEY",
            "AUTH_TOKEN", "WEBHOOK_SECRET", "PRIVATE_KEY", "API_KEY", "API_SECRET",
        )
...
def _fetch_activations(gateway_url: str, tools: list[dict]) -> list[dict]:
    """
    Call POST /v1/tools/{slug}/activate for each tool that has a slug.
    Returns list of activation response dicts (skips failures silently).
    """
    results = []
    for t in tools:
        slug = t.get("slug", "")
        ...
        try:
            resp = httpx.post(
                f"{gateway_url}/v1/tools/{slug}/activate",
                timeout=10.0,
            )
            if resp.status_code == 200:
...
def _rewrite_install_sh(out_path: "pathlib.Path", activations: list[dict]) -> None:
    """
    Rewrite install.sh using real install_cmd / deploy_cmd values from /activate.
    Groups commands by execution_type with inline comments.
    """
    import pathlib
    lines = ["#!/bin/bash", "# Generated by namango init — real commands from Namango catalog", "set -e", ""]
    ...
    for a in activations:
        ...
        exec_type = a.get("execution_type", "")
        steps     = a.get("next_steps", [])

        if exec_type == "install" and a.get("install_cmd"):
            install_cmds.append(f"# {name}")
            install_cmds.append(a["install_cmd"])
            install_cmds.append("")

        elif exec_type == "api" and a.get("install_cmd"):
            install_cmds.append(f"# {name} SDK")
            install_cmds.append(a["install_cmd"])
            install_cmds.append("")

        elif exec_type == "self-host":
            if a.get("deploy_cmd"):
                docker_cmds.append(f"# {name}")
                docker_cmds.append(a["deploy_cmd"])
                docker_cmds.append("")

        elif exec_type == "mcp" and a.get("install_cmd"):
            mcp_notes.append(f"# {name}")
            mcp_notes.append(a["install_cmd"])
            # MCP editor config hint
            editor = a.get("editor_config") or {}
            if editor.get("claude", {}).get("command"):
                slug = a.get("slug", "")
                cmd  = editor["claude"]["command"]
                mcp_notes.append(f"# Then add to Claude Code: claude mcp add {slug} --command {cmd}")
            mcp_notes.append("")

        elif exec_type in ("hosted", "recommend-only"):
            for step in steps:
                signup_notes.append(f"# {name}: {step}")

    if install_cmds:
        lines += ["# ── Install dependencies ──────────────────────────────────────"] + install_cmds
    if docker_cmds:
        lines += ["# ── Self-hosted services (Docker) ─────────────────────────────"] + docker_cmds
    if mcp_notes:
        lines += ["# ── MCP servers ───────────────────────────────────────────────"] + mcp_notes
    if signup_notes:
        lines += ["# ── Sign-up / hosted services (manual steps) ──────────────────"]
        lines += [f"echo '{n}'" if not n.startswith("#") else n for n in signup_notes]
        lines.append("")

    if not (install_cmds or docker_cmds or mcp_notes or signup_notes):
        lines.append("# No automated install commands found — check CLAUDE.md for setup instructions")

    (out_path / "install.sh").write_text("\n".join(lines) + "\n")
...
def _stream_blueprint(
    ...
    if full_response:
        ...
        # Enrich install.sh with real commands from /activate
        activations = _fetch_activations(gateway_url, final_tools)
        if activations:
            _rewrite_install_sh(out_path, activations)
